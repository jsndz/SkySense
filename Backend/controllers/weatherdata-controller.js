...
const DeleteWeatherData = async (req, res) => {
    try {
        const weatherdata = await weatherDataService.deleteWeatherData(req.params.id);
        return res.status(201).json({
            data: weatherdata,
            sucess: true,
            message: 'successfully created a WeatherData',
...
const UpdateWeatherData = async (req, res) => {
    try {
        const weatherdata = await weatherDataService.updateWeatherData(req.params.id,req.body);
        return res.status(201).json({
            data: weatherdata,
            sucess: true,
            message: 'successfully created a WeatherData',
